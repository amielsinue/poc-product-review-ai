...
class TestAnalyzeEndpoint:
    ...
    async def test_valid_review_returns_sentiment(self, async_client):
        response = await async_client.post("/analyze", json={"content": "Nice product"})
        assert response.status_code == 200
        data = response.json()
        assert data["sentiment"] == "positive"

    async def test_empty_review_returns_400(self, async_client):
        response = await async_client.post("/analyze", json={"content": "   "})
    ...
    async def test_review_returns_suggestions_list(self, async_client):
        response = await async_client.post("/analyze", json={"content": "bad product"})
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_neutral_review_returns_neutral_sentiment(self, async_client):
        response = await async_client.post(
            "/analyze", json={"content": "This is a product."}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_positive_review_has_no_suggestions(self, async_client):
        response = await async_client.post(
            "/analyze", json={"content": "I love this product. It works great!"}
        )
        assert response.status_code == 200
        data = response.json()
        assert data["sentiment"] == "positive"
    ...
    async def test_short_incomplete_review_has_multiple_suggestions(self, async_client):
        response = await async_client.post("/analyze", json={"content": "Bad"})
        assert response.status_code == 200
        data = response.json()
